...
public class Person {
    ...
    private void validateLogin(String login) {
        ...
        if (login.length() < 3 || login.length() > 30)
            throw new UncorrectException("Login must be between 3 and 30 characters long.", login);
        char[] cs = login.toCharArray();
        int cnt = 0;
        for (int i = 0; i < login.length(); i++) {
            if (Character.isWhitespace(cs[i])) cnt++;
            if (cnt >= 3) throw new UncorrectException("Login can contain up to 2 spaces.", login);
        }
    }
    ...
    private void validatePassword(String password) {
        ...
        if (password.length() < 8 || password.length() > 80)
            throw new UncorrectException("Password must be between 8 and 80 characters long.", password);
        ...
        if (!flagUpper || !flagLower || !flagDigit)
            throw new UncorrectException("Password must contain digits, upper and lower case characters.", password);
    }
    ...
    private void validatePhoneNumber(String phoneNumber) {
        ...
        if (phoneNumber.length() != 11 && phoneNumber.length() != 12)
            throw new UncorrectException("Phone number is uncorrect.", phoneNumber);
        ...
        for (int i = 1; i < phoneNumber.length(); i++) {
            if (!Character.isDigit(cs[i]))
                throw new UncorrectException("Phone number must contain only digits.", phoneNumber);
        }
    }
    ...
    private void validateEmail(String email) {
        ...
        if (ind == -1)
            throw new UncorrectException("Email must be format user@example.com.", email);
        ...
        for (int i = 0; i < email.length(); i++) {
            if (Character.isWhitespace(cs[i]))
                throw new UncorrectException("Email must not contain spaces.", email);
        }
    }
    ...
    private void validateSecretWord(String secretWord) {
        ...
        if (secretWord.length() < 3 || secretWord.length() > 80)
            throw new UncorrectException("Secret word must be between 3 and 80 characters long.", secretWord);
    }
